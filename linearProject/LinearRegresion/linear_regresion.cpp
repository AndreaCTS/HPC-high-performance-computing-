...
float linear_regresion:: F_OLS_Costo(Eigen::MatrixXd X, Eigen::MatrixXd y, Eigen::MatrixXd thetha)
{
...
std::tuple<Eigen::VectorXd, std::vector<float>> linear_regresion:: gradientDescent (Eigen::MatrixXd X,
                                                                                    Eigen::MatrixXd y,
...
/* A continuación se presenta la función para revisar que tan
 *bueno es nuestro modelo, se procede a crear la métrica de rendimiento:
 * R² score: coeficiente de determinación, en donde el mejor valor posible es 1*/

float linear_regresion:: R2score (Eigen::MatrixXd y, Eigen::MatrixXd y_hat){
        auto numerador = (pow((y- y_hat).array(),2)).sum();
        auto denominador = (pow(y.array() - y.mean(),2)).sum();
        return (1 - (numerador/denominador));
}

float linear_regresion:: MSE (Eigen::MatrixXd y, Eigen::MatrixXd y_hat ){
    // la matriz y son los valores reales y la matriz y_hat son los valores
    //predichos
    auto numerador = (pow(( y_hat - y).array(),2)).sum();
    auto denominador = y.rows();
    return (numerador / denominador);
}

float linear_regresion:: RMSE (Eigen::MatrixXd y, Eigen::MatrixXd y_hat ){
    // la matriz y son los valores reales y la matriz y_hat son los valores
    //predichos
    auto numerador = (pow(( y_hat - y).array(),2)).sum();
    auto denominador = y.rows();
...
float linear_regresion:: CoeficienteDeterminados (Eigen::MatrixXd y, Eigen::MatrixXd y_hat ){
    // la matriz y son los valores reales y la matriz y_hat son los valores
    //predichos
    auto numerador = (pow(( y_hat.array() - y.mean()),2)).sum();
    auto denominador = (pow(( y.array() - y.mean()),2)).sum();
    return (numerador / denominador);
}
